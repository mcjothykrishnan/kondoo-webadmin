...
const getAge = (date) => {
  if (date) {
    ...
    const monthDiff = moment().diff(moment(date, 'DD/MM/YYYY'), 'months', false);
    if (monthDiff > 0) {
      return `${monthDiff} M`;
    }
...
const makeName = (nameObj = {}) => {
  ...
  const prefixVal =
    prefix?.[0]?.coding?.[0]?.display || prefix?.coding?.[0]?.display || prefix?.[0]?.display || prefix?.display || '';
  const suffixVal =
    suffix?.[0]?.coding?.[0]?.display || suffix?.coding?.[0]?.display || suffix?.[0]?.display || suffix?.display || '';
  return `${prefixVal && `${prefixVal}.`}${text && `${text} `}${given && `${given} `}${family && `${family} `}${
    suffixVal && suffixVal
  }`;
};
...
export { getAge, validateNetworkError, returnException, fetchData, getUtcTime, utcTOLocal, makeName };
